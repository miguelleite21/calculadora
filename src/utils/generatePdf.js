...
export const generatePdf = async (name, images, pieces, date, customFabrics = []) => {
  const doc = new jsPDF({
  ...
  const addFooter = () => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(8);
      doc.text(
        `Pink Paradise Ateliê - Página ${i} de ${pageCount}`,
  ...
  const tableBody = pieces.map(piece => {
    ...
    piece.details.forEach(d => {
      ...
      const isCloth = allFabrics.some(c => c.name === d.name);
      if (isCloth) {
        tecidosSum += d.value;
      } else {
